"""
Comprehensive tests for the 1000-book embedding sample.
...
def test_integrity(meta: np.ndarray, review: np.ndarray, titles: list[str]) -> None:
    ...
    check("All 1000 books have metadata_embedding", len(meta) == 1000,
          f"got {len(meta)}")
    ...
    meta_normed   = np.allclose(meta_norms,   1.0, atol=1e-4)
    review_normed = np.allclose(review_norms, 1.0, atol=1e-4)
    check("Metadata vectors are unit-normalized",
          meta_normed, f"mean norm={meta_norms.mean():.6f}")
    check("Review vectors are unit-normalized",
          review_normed, f"mean norm={review_norms.mean():.6f}")
...
def test_geometry(meta: np.ndarray, review: np.ndarray, genres: list[list]) -> None:
    ...
    # Pairwise cosine on a random 200-book sample (avoid O(n^2) on all 1000)
    ...
    check("Mean inter-book similarity < 0.95 (no collapse)",
          flat.mean() < 0.95, f"mean={flat.mean():.4f}")
    check("Mean inter-book similarity > 0.0 (not random noise)",
          flat.mean() > 0.0, f"mean={flat.mean():.4f}")

    # Genre separation: pick two well-populated genres and compare within vs across
    genre_map: dict[str, list[int]] = {}
    for i, g_list in enumerate(genres):
        for g in (g_list or []):
            genre_map.setdefault(g, []).append(i)

    # Find two genres with >= 10 books each
    candidates = [(g, idxs) for g, idxs in genre_map.items() if len(idxs) >= 10]
    candidates.sort(key=lambda x: -len(x[1]))

    if len(candidates) >= 2:
        g1_name, g1_idx = candidates[0]
        # Pick second genre with minimal overlap
        for g2_name, g2_idx in candidates[1:]:
            overlap = len(set(g1_idx) & set(g2_idx))
            if overlap < 3:
                break
        ...
        across = g1_vecs @ g2_vecs.T
        across_mean = across.mean()

        print(f"\n  Genre separation: '{g1_name}' ({len(g1_idx)} books) vs "
              f"'{g2_name}' ({len(g2_idx)} books)")
        print(f"    within-genre cosine: {within_mean:.4f}")
        print(f"    across-genre cosine: {across_mean:.4f}")
        check("Within-genre similarity > across-genre similarity",
...
def test_semantic(
    ...
    for query_text, expected_genres in queries:
        ...
        # Genre hit: does any top-5 result have a genre in expected_genres?
        ...
        meta_hit   = genre_hit(top5_meta)
        review_hit = genre_hit(top5_review)

        print(f"\n  Query: '{query_text}'")
        print(f"    Top-5 metadata results:")
        for i in top5_meta:
            g = ", ".join((genres[i] or [])[:3])
            print(f"      [{meta_scores[i]:.4f}] {titles[i][:60]}  [{g}]")
        print(f"    Top-5 review results:")
        for i in top5_review:
            g = ", ".join((genres[i] or [])[:3])
            print(f"      [{review_scores[i]:.4f}] {titles[i][:60]}  [{g}]")

        check(f"Metadata top-5 genre hit for '{query_text}'",   meta_hit)
        check(f"Review   top-5 genre hit for '{query_text}'", review_hit)
...
def test_consistency(meta: np.ndarray, review: np.ndarray, titles: list[str]) -> None:
    print("\n=== 4. CONSISTENCY (metadata vs review agreement) ===")

    # Per-book cosine between its own metadata and review embedding
    per_book_sim = np.sum(meta * review, axis=1)  # dot of unit vectors = cosine
    ...
    check("Mean self-similarity > 0.2 (embeddings agree at least loosely)",
          per_book_sim.mean() > 0.2, f"mean={per_book_sim.mean():.4f}")

    # Nearest neighbor overlap: for each book, how many of its top-10 metadata
    # neighbors also appear in its top-10 review neighbors?
    meta_top10   = np.argsort(-(meta   @ meta.T),   axis=1)[:, 1:11]
    review_top10 = np.argsort(-(review @ review.T), axis=1)[:, 1:11]
    overlaps = [
        len(set(meta_top10[i]) & set(review_top10[i]))
        for i in range(len(titles))
    ]
    mean_overlap = np.mean(overlaps)
    print(f"\n  Top-10 neighbor overlap (metadata vs review): mean={mean_overlap:.2f}/10")
    check("Mean neighbor overlap > 1.0 (embeddings share some signal)",
          mean_overlap > 1.0, f"mean={mean_overlap:.2f}")

    # Books where embeddings disagree most
...
def main() -> None:
    ...
    def parse_vec(s: str | None) -> np.ndarray:
        if s is None:
            return np.zeros(DIM)
